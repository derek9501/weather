...
const CWA_AUTH_KEY = process.env.CWA_AUTH_KEY;

// 氣象署熱帶氣旋路徑與預報資料 API URL (安全帶入 Secrets 授權碼)
const CWA_TYPHOON_TRACK_API = `https://opendata.cwa.gov.tw/api/v1/rest/datastore/W-C0034-005?Authorization=${CWA_AUTH_KEY}&format=XML`;
...
function loadDatabase() {
  ...
  if (fs.existsSync(DATA_FILE_PATH)) {
    try {
      ...
      console.error("資料庫讀取失敗，自動初始化新資料庫");
    }
  }
  return {
    queue: { currentStep: 0 },
